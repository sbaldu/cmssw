...
void PatternRecognitionByCLUEstering::makeTracksters(
    Queue &queue, const reco::CaloClusterDeviceCollection &lc,
  ...
  if (n > 0) {
    auto d_clIndex = cms::alpakatools::make_device_buffer<int[]>(
        queue, n); // temporary buffer needed by CLUEstering
    ...
    for (auto i = 0ul; i < tsMap.size(); ++i) {
      ...
      tracksters[i].vertex_multiplicity().resize(
          tracksters[i].vertices().size(), 1);
    }

    alpaka::memcpy(
        queue, cms::alpakatools::make_host_view(h_points.view().coords()[0], n),
        cms::alpakatools::make_device_view(alpaka::getDev(queue), x, n),
        static_cast<uint32_t>(n));
    alpaka::memcpy(
        queue, cms::alpakatools::make_host_view(h_points.view().coords()[1], n),
        cms::alpakatools::make_device_view(alpaka::getDev(queue), y, n),
        static_cast<uint32_t>(n));
    alpaka::memcpy(
        queue, cms::alpakatools::make_host_view(h_points.view().coords()[2], n),
        cms::alpakatools::make_device_view(alpaka::getDev(queue), z, n),
        static_cast<uint32_t>(n));
    alpaka::memcpy(
        queue, cms::alpakatools::make_host_view(h_points.weights(), n),
    ...
    for (auto &trackster : tracksters) {
      ...
      auto fillPoint = [&](const float x, const float y, const float z,
...
void PatternRecognitionByCLUEstering::fillPSetDescription(
