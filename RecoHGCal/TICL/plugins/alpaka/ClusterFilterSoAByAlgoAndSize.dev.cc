...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  public:
    ...
    ALPAKA_FN_ACC void operator()(TAcc const& acc,
                                  ...
                                  const int max_cluster_size) const {
      for (int32_t i : cms::alpakatools::uniform_elements(acc, layerClustersSoA.metadata().size())) {
        ...
        if (layerClustersSoA[i].cells() > max_cluster_size or
            ((layerClustersSoA[i].cells() < min_cluster_size) and
             (detid.det() == DetId::HGCalEE or detid.det() == DetId::HGCalHSi or
  ...
  void ClusterFilterSoAByAlgoAndSize::filter(Queue& queue,
    ...
    alpaka::exec<Acc1D>(
        queue, workDiv, FilterKernel{}, layerClusters, layerClustersMask, min_cluster_size, max_cluster_size);
  }
