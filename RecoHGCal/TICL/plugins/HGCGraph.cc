...
void HGCGraphT<TILES>::makeAndConnectDoublets(
    const TILES &histo, const std::vector<TICLSeedingRegion> &regions,
  ...
  isOuterClusterOfDoublets_.resize(layerClusters.metadata().size()[0]);
  allDoublets_.clear();
  theRootDoublets_.clear();
  bool checkDistanceRootDoubletVsSeed =
  ...
  for (const auto &r : regions) {
    ...
    if (isGlobal) {
      ...
      if (verbosity_ > ticl::VerbosityLevel::Guru) {
        LogDebug("HGCGraph")
            << " Entrance eta, phi: " << origin_eta << ", " << origin_phi
            << " entryEtaBin: " << entryEtaBin
            << " entryPhiBin: " << entryPhiBin << " globalBin: "
            << firstLayerHisto.globalBin(origin_eta, origin_phi)
            << " on layer: " << firstLayerOnZSide
            << " startEtaBin: " << startEtaBin << " endEtaBin: " << endEtaBin
            << " startPhiBin: " << startPhiBin << " endPhiBin: " << endPhiBin
            << " phiBin(0): " << firstLayerHisto.phiBin(0.) << " phiBin("
            << M_PI / 2. << "): " << firstLayerHisto.phiBin(M_PI / 2.)
            << " phiBin(" << M_PI << "): " << firstLayerHisto.phiBin(M_PI)
            << " phiBin(" << -M_PI / 2.
            << "): " << firstLayerHisto.phiBin(-M_PI / 2.) << " phiBin("
            << -M_PI << "): " << firstLayerHisto.phiBin(-M_PI) << " phiBin("
            << 2. * M_PI << "): " << firstLayerHisto.phiBin(2. * M_PI)
            << std::endl;
      }
    }
    ...
    for (int il = 0; il < maxNumberOfLayers - 1; ++il) {
      for (int outer_layer = 0;
           outer_layer < std::min(1 + skip_layers, maxNumberOfLayers - 1 - il);
           ++outer_layer) {
        ...
        const int etaLimitIncreaseWindowBin =
            innerLayerHisto.etaBin(etaLimitIncreaseWindow);
          ...
          for (int iphi_it = startPhiBin; iphi_it < endPhiBin; ++iphi_it) {
            ...
            for (auto innerClusterId : innerLayerHisto[offset + iphi]) {
              ...
              if (mask[innerClusterId] == 0.) {
                if (verbosity_ > ticl::VerbosityLevel::Advanced)
                  LogDebug("HGCGraph") << "Skipping inner masked cluster "
                                       << innerClusterId << std::endl;
                continue;
              }

              // For global-seeded iterations, if the inner cluster is above a
              // certain eta-threshold, i.e., it has higher eta, make the outer
              // search window bigger in both eta and phi by one bin, to contain
              // better low energy showers. Track-Seeded iterations are excluded
              // since, in that case, the inner search window has already been
              // enlarged.
              auto etaWindow = deltaIEta;
              auto phiWindow = deltaIPhi;
              ...
              for (int oeta = etaRangeMin; oeta < etaRangeMax; ++oeta) {
                // wrap phi bin
                for (int phiRange = 0; phiRange < 2 * phiWindow + 1;
                     ++phiRange) {
                  ...
                  auto ophi =
                      ((iphi + phiRange - phiWindow) % nPhiBins + nPhiBins) %
                      nPhiBins;
                  if (verbosity_ > ticl::VerbosityLevel::Guru) {
                    LogDebug("HGCGraph")
                        << "Outer Global Bin: " << (oeta * nPhiBins + ophi)
                        << " on layers I/O: " << currentInnerLayerId << "/"
                        << currentOuterLayerId << " with clusters: "
                        << innerLayerHisto[oeta * nPhiBins + ophi].size()
                        << std::endl;
                  }
                  for (auto outerClusterId :
                       outerLayerHisto[oeta * nPhiBins + ophi]) {
                    // Skip masked clusters
                    if (mask[outerClusterId] == 0.) {
                      if (verbosity_ > ticl::VerbosityLevel::Advanced)
                        LogDebug("HGCGraph") << "Skipping outer masked cluster "
                                             << outerClusterId << std::endl;
                      continue;
                    }
                    auto doubletId = allDoublets_.size();
                    if (maxDeltaTime != -1 &&
                        !areTimeCompatible(innerClusterId, outerClusterId,
                                           layerClusters, maxDeltaTime)) {
                      if (verbosity_ > ticl::VerbosityLevel::Advanced)
                        LogDebug("HGCGraph")
                    ...
                    if (currentOuterLayerId - currentInnerLayerId == 1) {
                      if (areOverlappingOnSiblingLayers(
                              innerClusterId, outerClusterId, layerClusters,
                              maxRSquared)) {
                        allDoublets_.emplace_back(
                            innerClusterId, outerClusterId, doubletId,
                            layerClusters, r.index, true);
                      } else {
                        continue;
                      }
                    } else {
                      allDoublets_.emplace_back(innerClusterId, outerClusterId,
                                                doubletId, layerClusters,
                                                r.index, false);
                    }
                    if (verbosity_ > ticl::VerbosityLevel::Advanced) {
                      LogDebug("HGCGraph")
                    ...
                    isOuterClusterOfDoublets_[outerClusterId].push_back(
                        doubletId);
                    auto &neigDoublets =
                        isOuterClusterOfDoublets_[innerClusterId];
                    auto &thisDoublet = allDoublets_[doubletId];
                    if (verbosity_ > ticl::VerbosityLevel::Expert) {
                      LogDebug("HGCGraph")
                          << "Checking compatibility of doubletId: "
                          << doubletId
                          << " with all possible inners doublets link by the "
                             "innerClusterId: "
                          << innerClusterId << std::endl;
                    }
                    bool isRootDoublet = thisDoublet.checkCompatibilityAndTag(
                        allDoublets_, neigDoublets, r.directionAtOrigin,
                        minCosTheta, minCosPointing,
  ...
  if (verbosity_ > ticl::VerbosityLevel::None) {
    LogDebug("HGCGraph") << "number of Root doublets "
...
template <typename TILES>
bool HGCGraphT<TILES>::areTimeCompatible(
  ...
  return (timeIn == -99. || timeOut == -99. ||
          std::abs(timeIn - timeOut) <
              maxDeltaTime * sqrt(timeInE * timeInE + timeOutE * timeOutE));
}

template <typename TILES>
bool HGCGraphT<TILES>::areOverlappingOnSiblingLayers(
    int innerIdx, int outerIdx,
    const reco::CaloClusterHostCollection::ConstView &layerClusters,
    float maxRSquared) {
  return reco::deltaR2(layerClusters.eta(outerIdx), layerClusters.phi(outerIdx),
...
template <typename TILES>
void HGCGraphT<TILES>::findNtuplets(
    std::vector<HGCDoublet::HGCntuplet> &foundNtuplets,
    std::vector<int> &seedIndices, const unsigned int minClustersPerNtuplet,
  ...
  for (auto rootDoublet : theRootDoublets_) {
    ...
    allDoublets_[rootDoublet].findNtuplets(allDoublets_, tmpNtuplet, seedIndex,
                                           outInDFS, outInHops, maxOutInHops,
                                           outInToVisit);
