...
void ticl::assignPCAtoTracksters(std::vector<Trackster> &tracksters,
  ...
  for (auto &trackster : tracksters) {
    ...
    for (size_t i = 0; i < N; ++i) {
      ...
      bool isEM = false;
      if (isBarrel) {
        auto x2 = std::pow(clusters.position()[trackster.vertices(i)].x(), 2);
        auto y2 = std::pow(clusters.position()[trackster.vertices(i)].y(), 2);
        isEM = std::sqrt(x2 + y2) < z_limit_em;
      } else {
        isEM = std::abs(clusters.position()[trackster.vertices(i)].z()) <= z_limit_em;
      }
      if (isEM)
