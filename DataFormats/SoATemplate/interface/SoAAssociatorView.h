...
#define _DECLARE_SOA_ASSOCIATOR_MEMBERS_ACCESSORS_IMPL(LAYOUT_NAME, LAYOUT_MEMBER, LOCAL_NAME) \
  SOA_HOST_DEVICE auto LOCAL_NAME(size_t assoc_id) {                                           \
    auto offset = BOOST_PP_CAT(LOCAL_NAME, _) + offsets_[assoc_id];                            \
    auto size = offsets_[assoc_id + 1] - offsets_[assoc_id];                                   \
                                                                                               \
    std::cout << "pippo" << std::endl;                                                         \
    return std::span(offset, size);                                                            \
  }                                                                                            \
  SOA_HOST_DEVICE auto LOCAL_NAME(size_t assoc_id) const {                                     \
    auto offset = BOOST_PP_CAT(LOCAL_NAME, _) + offsets_[assoc_id];                            \
    auto size = offsets_[assoc_id + 1] - offsets_[assoc_id];                                   \
                                                                                               \
    std::cout << "pippo" << std::endl;                                                         \
...
#define _DECLARE_SOA_ASSOCIATOR_MEMBERS_ACCESSORS(R, DATA, TYPE_NAME) \
  BOOST_PP_EXPAND(_DECLARE_SOA_ASSOCIATOR_MEMBERS_ACCESSORS_IMPL TYPE_NAME)
...
#define _DECLARE_VIEW_MEMBER_INITIALIZERS_IMPL(LAYOUT, MEMBER, NAME)                                                   \
  (BOOST_PP_CAT(NAME, Parameters_)([&]() -> auto {                                                                     \
    auto params = instance_Associator.metadata().BOOST_PP_CAT(parametersOf_, MEMBER)();                                             \
...
// clang-format off
#define _UPDATE_BINS_OF_VIEW_IMPL(LAYOUT_TYPE, LAYOUT_NAME)                                                            \
  if (set) {                                                                                                           \
    if (ret != LAYOUT_NAME.metadata().bins())                                                                          \
      throw std::runtime_error("In constructor by layout: different sizes from layouts.");                             \
  } else {                                                                                                             \
    ret = LAYOUT_NAME.metadata().bins();                                                                               \
    set = true;                                                                                                        \
  }

// clang-format on
#define _UPDATE_BINS_OF_VIEW(R, DATA, TYPE_NAME) BOOST_PP_EXPAND(_UPDATE_BINS_OF_VIEW_IMPL TYPE_NAME)
...
#define _DECLARE_VIEW_SOA_ACCESSOR_IMPL(LAYOUT_NAME, LAYOUT_MEMBER, LOCAL_NAME)                                        \
  ...
  LOCAL_NAME(size_type _soa_impl_index) {                                                                              \
    if constexpr (rangeChecking == cms::soa::RangeChecking::enabled) {                                                 \
      if (_soa_impl_index >= base_type::associations_ or _soa_impl_index < 0)                                              \
...
#define _DECLARE_VIEW_SOA_CONST_ACCESSOR_IMPL(LAYOUT_NAME, LAYOUT_MEMBER, LOCAL_NAME)                                  \
  ...
  LOCAL_NAME(size_type _soa_impl_index) const {                                                                        \
    if constexpr (rangeChecking == cms::soa::RangeChecking::enabled) {                                                 \
      if (_soa_impl_index >= associations_ or _soa_impl_index < 0)                                                     \
...
#define _GENERATE_SOA_VIEW_PART_1(CONST_VIEW, VIEW, LAYOUTS_LIST, VALUE_LIST)                                          \
    ...
    struct Metadata {                                                                                                  \
      friend VIEW;                                                                                                     \
      SOA_HOST_DEVICE SOA_INLINE size_type size() const { return parent_.associations_; }                              \
	  SOA_HOST_DEVICE SOA_INLINE size_type bins() const { return parent_.bins_; }									   \
	  ...
	  SOA_HOST_DEVICE SOA_INLINE uint32_t* offsets() { return const_cast<uint32_t*>(parent_.offsets_.addr_); }								       \
	  SOA_HOST_DEVICE SOA_INLINE const uint32_t* offsets() const { return parent_.offsets_.addr_; }						   \
                                                                                                                       \
...
#define _GENERATE_SOA_CONST_VIEW_PART_1(CONST_VIEW, VIEW, LAYOUTS_LIST, VALUE_LIST)                                    \
    ...
    struct Metadata {                                                                                                  \
      ...
      /* Alias layout or view types to name-derived identifyer to allow simpler definitions */                         \
      _ITERATE_ON_ALL(_DECLARE_VIEW_LAYOUT_TYPE_ALIAS, ~, LAYOUTS_LIST)                                                \
	  SOA_HOST_DEVICE SOA_INLINE uint32_t* offsets() { return parent_.offsets_.addr_; }								       \
	  SOA_HOST_DEVICE SOA_INLINE const uint32_t* offsets() const { return parent_.offsets_.addr_; }						   \
                                                                                                                       \
      /* Alias member types to name-derived identifyer to allow simpler definitions */                                 \
      _ITERATE_ON_ALL(_DECLARE_VIEW_MEMBER_TYPE_ALIAS, BOOST_PP_EMPTY(), VALUE_LIST)                                   \
	  using TypeOf_offsets =                                                                            			   \
		  typename TypeOf_instance_Associator::Metadata::TypeOf_offsets;                     						   \
	  using ParametersTypeOf_offsets =                                                                  			   \
		  typename TypeOf_instance_Associator::Metadata::ParametersTypeOf_offsets;           				   		   \
	  constexpr static cms::soa::SoAColumnType ColumnTypeOf_offsets =                               				   \
		  TypeOf_instance_Associator::Metadata::ColumnTypeOf_offsets;                        						   \
	  using ConstAccessorOf_offsets =                                                                   			   \
		typename cms::soa::SoAAccessors<typename Metadata::TypeOf_offsets>::                        				   \
			template ColumnType<ColumnTypeOf_offsets>::template AccessType<                         				   \
				cms::soa::SoAAccessType::constAccess>::template Alignment<conditionalAlignment>::                      \
					template RestrictQualifier<restrictQualify> ;                                                      \
	  using MutableAccessorOf_offsets =                                                                 			   \
		typename cms::soa::SoAAccessors<typename Metadata::TypeOf_offsets>::                        				   \
			template ColumnType<ColumnTypeOf_offsets>::template AccessType<                         				   \
				cms::soa::SoAAccessType::mutableAccess>::template Alignment<conditionalAlignment>::                    \
					template RestrictQualifier<restrictQualify> ;                                                      \
	  SOA_HOST_DEVICE SOA_INLINE                                                                                       \
	  const auto parametersOf_offsets() const {                                                         			   \
		return CAST(parent_.offsetsParameters_);                                                        			   \
	  }																											   	   \
																													   \
      _ITERATE_ON_ALL(_DECLARE_VIEW_MEMBER_CONST_POINTERS, ~, VALUE_LIST)                                              \
                                                                                                                       \
    ...
    SOA_HOST_ONLY CONST_VIEW(const Associator& instance_Associator)        \
        : associations_([&]() -> size_type {                                                                           \
            bool set = false;                                                                                          \
            size_type ret = 0;                                                                                         \
            ...
            bool set = false;                                                                                          \
            size_type ret = 0;                                                                                         \
            _ITERATE_ON_ALL(_UPDATE_BINS_OF_VIEW, BOOST_PP_EMPTY(), LAYOUTS_LIST)                                      \
            return ret;                                                                                                \
          }()),                                                                                                        \
          _ITERATE_ON_ALL_COMMA(_DECLARE_VIEW_MEMBER_INITIALIZERS, ~, VALUE_LIST),									   \
		  offsets_([&]() -> auto {                                                                     				   \
			if constexpr (alignmentEnforcement == AlignmentEnforcement::enforced)                                      \
			  if (reinterpret_cast<intptr_t>(offsets_) % alignment)                                                	   \
				throw std::runtime_error("In constructor by layout: misaligned column: offsets");                      \
			return instance_Associator.metadata().parametersOf_offsets();                                                           \
		  }()) {}                                   																   \
                                                                                                                       \
    /* Constructor relying on individually provided column addresses */                                                \
    SOA_HOST_ONLY CONST_VIEW(size_type _soa_impl_associations,														   \
							 size_type _soa_impl_bins,															       \
                        _ITERATE_ON_ALL_COMMA(_DECLARE_VIEW_CONSTRUCTION_BYCOLUMN_PARAMETERS, const, VALUE_LIST))      \
        : associations_(_soa_impl_associations), bins_(_soa_impl_bins),                                                \
          _ITERATE_ON_ALL_COMMA(_DECLARE_VIEW_MEMBER_INITIALIZERS_BYCOLUMN, ~, VALUE_LIST),							   \
		  offsets_([&]() -> auto {                                                                    	   			   \
			if constexpr (alignmentEnforcement == AlignmentEnforcement::enforced)                                      \
			  if (Metadata::ParametersTypeOf_offsets::checkAlignment(offsets_, alignment))                   		   \
				throw std::runtime_error("In constructor by column: misaligned column: offsets" );                     \
			return offsets_;                                                                                           \
		  }()) {}                          																			   \
                                                                                                                       \
    /* Copiable */                                                                                                     \
    ...
    SOA_HOST_ONLY friend void dump();                                                                                  \
																													   \
	_ITERATE_ON_ALL(_DECLARE_SOA_ASSOCIATOR_MEMBERS_ACCESSORS, ~, VALUE_LIST)										   \
                                                                                                                       \
  private:                                                                                                             \
    size_type associations_ = 0;                                                                                       \
    size_type bins_ = 0;                                                                                           	   \
    _ITERATE_ON_ALL(_DECLARE_CONST_VIEW_SOA_MEMBER, const, VALUE_LIST)                                                 \
	Metadata::ParametersTypeOf_offsets::ConstType offsets_;															   \
};
// clang-format on
