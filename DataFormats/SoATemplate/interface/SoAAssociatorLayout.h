#ifndef DataFormats_SoATemplate_interface_SoAAssociatorLayout_h
#define DataFormats_SoATemplate_interface_SoAAssociatorLayout_h

#include "FWCore/Reflection/interface/reflex.h"

#include "SoACommon.h"
#include "SoAAssociatorView.h"
...
#define _ASSIGN_SOA_COLUMN_OR_SCALAR_IMPL(VALUE_TYPE, CPP_TYPE, NAME)                                                  \
  _SWITCH_ON_TYPE(VALUE_TYPE,                                                                                          \
      ...
      /* Column */                                                                                                     \
      BOOST_PP_CAT(NAME, _) = reinterpret_cast<CPP_TYPE*>(_soa_impl_curMem);                                           \
      _soa_impl_curMem += cms::soa::alignSize(associations_ * sizeof(CPP_TYPE), alignment);                            \
      ,                                                                                                                \
      /* Eigen column */                                                                                               \
      BOOST_PP_CAT(NAME, Stride_) = cms::soa::alignSize(associations_ * sizeof(CPP_TYPE::Scalar), alignment)           \
...
#define _ACCUMULATE_SOA_ELEMENT_IMPL(VALUE_TYPE, CPP_TYPE, NAME)                                                       \
  _SWITCH_ON_TYPE(VALUE_TYPE,                                                                                          \
      ...
      ,                                                                                                                \
      /* Column */                                                                                                     \
      _soa_impl_ret += cms::soa::alignSize(associations * sizeof(CPP_TYPE), alignment);                                \
      ,                                                                                                                \
      /* Eigen column */                                                                                               \
      _soa_impl_ret += cms::soa::alignSize(associations * sizeof(CPP_TYPE::Scalar), alignment)                         \
             * CPP_TYPE::RowsAtCompileTime * CPP_TYPE::ColsAtCompileTime;                                              \
  )
// clang-format on
...
// clang-format off
#define GENERATE_SOA_ASSOCIATOR_LAYOUT(CLASS, ...)                                                                     \
  ...
  struct CLASS {                                                                                                       \
    ...
    static constexpr byte_size_type computeDataSize(size_type associations) {                                          \
    ...
    struct Metadata {                                                                                                  \
	  ...
	  SOA_HOST_DEVICE SOA_INLINE uint32_t* offsets() { return parent_.offsets_; }									   \
	  SOA_HOST_DEVICE SOA_INLINE const uint32_t* offsets() const { return parent_.offsets_; }						   \
      ...
      SOA_HOST_DEVICE SOA_INLINE CLASS cloneToNewAddress(std::byte* _soa_impl_addr) const {                            \
        return CLASS(_soa_impl_addr, parent_.associations_);                                                           \
      }                                                                                                                \
                                                                                                                       \
      _ITERATE_ON_ALL(_DEFINE_METADATA_MEMBERS, ~, __VA_ARGS__)                                                        \
	  using ParametersTypeOf_offsets =                                                                    			   \
         cms::soa::SoAParameters_ColumnType<cms::soa::SoAColumnType::column>::DataType<uint32_t>;                      \
      SOA_HOST_DEVICE SOA_INLINE                                                                                       \
      ParametersTypeOf_offsets parametersOf_offsets() const {                                						   \
        return ParametersTypeOf_offsets (parent_.offsets_);                                 						   \
      }                                                                                                                \
      SOA_HOST_DEVICE SOA_INLINE                                                                                       \
      uint32_t const* addressOf_offsets() const {                                                         			   \
        return parent_.metadata().parametersOf_offsets().addr_;                                           			   \
      }                                                                                                                \
      SOA_HOST_DEVICE SOA_INLINE                                                                                       \
      uint32_t* addressOf_offsets() {                                                                     			   \
        return parent_.metadata().parametersOf_offsets().addr_;                                           			   \
      }                                                                                                                \
      SOA_HOST_DEVICE SOA_INLINE                                                                                       \
      byte_size_type offsetsPitch() const {                                                               			   \
        return cms::soa::alignSize(parent_.associations_ * sizeof(uint32_t), ParentClass::alignment);                      \
      }                                                                                                                \
      using TypeOf_offsets = uint32_t;                                                                    			   \
      constexpr static cms::soa::SoAColumnType ColumnTypeOf_offsets = cms::soa::SoAColumnType::column;   			   \
                                                                                                                       \
    ...
    SOA_HOST_ONLY CLASS(std::byte* mem, size_type associations, size_type bins) :									   \
	  mem_(mem), associations_(associations), bins_(bins), byteSize_(0) {           								   \
    ...
    SOA_HOST_ONLY CLASS(CLASS const& _soa_impl_other)                                                                  \
        : mem_(_soa_impl_other.mem_),                                                                                  \
          associations_(_soa_impl_other.associations_),                                                                \
    ...
    template <typename T>                                                                                              \
    SOA_HOST_DEVICE friend void dump();                                                                                \
                                                                                                                       \
  private:                                                                                                             \
    ...
    void organizeColumnsFromBuffer() {                                                                                 \
	  ...
	  offsets_ = reinterpret_cast<uint32_t*>(_soa_impl_curMem);                                           			   \
      /* Sanity check: we should have reached the computed size, only on host code */                                  \
      byteSize_ = computeDataSize(associations_);                                                                      \
